...
void __TreeMap_put(struct TreeMap* self, char *key, int value) {
    ...
    if(right != NULL && right->__left == NULL) right->__left = new;
    else if(left != NULL && left->__right == NULL) left->__right = new;
    else printf("Tree insertion error\n");
...
struct TreeMapEntry* __TreeMapIter_next(struct TreeMapIter* self){
     ...
     */
    struct TreeMapEntry* current = self->__current;
